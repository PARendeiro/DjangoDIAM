...
def criarquestao(request):
    if request.method == 'POST':
        try:
            ...
            q = Questao(questao, pub_data)
            q.save()
            return render(request, 'votacao/index.html')
        except:
            return render(request, 'criarquestao.html', { 'erro': 'Algo de errado ocorreu com a questão!'})
    else:
        return render(request, 'criarquestao.html', {})


    #string = get_object_or_404(string2)
    #questao = Questao(questao_texto=string, pub_data=timezone.now())
    #questao.save()
